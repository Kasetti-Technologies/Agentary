...
const PricingEngine_1 = require("./PricingEngine"); // adjust path if needed
async function main() {
    ...
    const engine = new PricingEngine_1.PricingEngine(pool);
    // 3) Define test input
    ...
    try {
        ...
        const result = await engine.resolvePrice({
            ...
            quantity,
            effectiveAt,
        }); // 'as any' to bypass TS mismatch if your type names differ slightly
        ...
        if (result.unitPriceCents !== undefined &&
            result.amountCents !== undefined &&
            result.priceId) {
            const unit = result.unitPriceCents;
            const amount = result.amountCents;
            const priceId = result.priceId;
            const source = result.source || 'unknown';
            console.log('');
            console.log(`Price ID    : ${priceId} (source: ${source}, e.g. override/promo/volume/base)`);
